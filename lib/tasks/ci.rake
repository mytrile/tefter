...
namespace :ci do
  ...
  task :symlink_db_config do
    sh "cp ../../config/database.yml.#{ENV['DEPLOY_TARGET']} config/database.yml"
  end

  desc "Deploy to current environment"
  task :deploy do
    sh "cap #{ENV['DEPLOY_TARGET']} deploy:migrations"
  end
...
task :ci => ['ci:symlink_db_config', 'db:migrate', 'ci:acceptance', 'ci:deploy']
